//gcc -Wall -pedantic UDP_client.c -l ws2_32 -o UDP_client
#ifdef _WIN32
...
void execution( int internet_socket, struct sockaddr * internet_address, socklen_t internet_address_length )
{
	...
	while (1)
	{
	char aantalPackets[3] = {0};
		printf("amount of packets: ");
		scanf("%s",aantalPackets);
		if(isdigit( (int ) aantalPackets[0]))
		{
				
				int recievedPackages = 0;
			int number_of_bytes_send = 0;
			number_of_bytes_send = sendto( internet_socket, aantalPackets, sizeof(aantalPackets) + 1, 0, internet_address, internet_address_length );
			...
			while( recievedPackages < atoi(aantalPackets));
			printf("%d of the %d received \n",recievedPackages, atoi(aantalPackets));
			
			
			if (recievedPackages == atoi(aantalPackets))
			{
				int number_of_bytes_send = 0;
				
				number_of_bytes_send = sendto( internet_socket, (char *) "packets received", 16, 0, internet_address, internet_address_length );
				if( number_of_bytes_send == -1 )
				{
					perror( "sendto" );
					return;
		...
		{
			system("CLS");
			printf("user input for packets = %s\n",aantalPackets);
			printf("give a number for the amount of packets\n");
